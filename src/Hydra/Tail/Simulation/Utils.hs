...
import Control.Monad.IOSim (SimEvent, SimEventType (..), SimResult, ThreadLabel, Trace (..))
...
foldTraceEvents fn !st = \case
  SimTrace time threadId mThreadLabel (EventLog event) next -> do
  ...
  SimTrace _time _threadId _threadLabel (EventThrow e) _next ->
    throw e
  SimTrace _time _threadId _threadLabel _event next ->
    foldTraceEvents fn st next
  TraceMainReturn{} ->
    pure st
