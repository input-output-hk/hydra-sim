...
/* Generate a T2P2 dataset from the "Social circles: Twitter" dataset (https://snap.stanford.edu/data/ego-Twitter.html).
 *
 * Usage:
 *     ./twitter.mjs
 *     PROACTIVE=0.8 ./twitter.mjs
 *
 * Available ENV vars:
 *
...
const INPUT_FILENAME = path.join(__dirname, "twitter", "source.csv");
...
fs.createReadStream(INPUT_FILENAME)
  .pipe(forEachLine(cols => {
    ...
    graph.push({
      source: nodes.get(cols[0]),
      target: nodes.get(cols[1])
    })
  }))
  .on('finish', () => {
